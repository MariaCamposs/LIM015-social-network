...
import {
  getPosts, getUserToFollow, getUserWithOnSnapshot, updatePost, deletePost,
} from '../firebase/firestore.js';
import { preventDeleteModal, postRemovedModal } from './modal.js';
...
const addMoreActions = (idPost, dataPost, user) => {
  ...
  deletePostButton.addEventListener('click', () => {
    home.appendChild(preventDeleteModal());
    const acceptButton = home.querySelector('#delete');
    acceptButton.addEventListener('click', async () => {
      home.querySelector('.prevent-delete').style.display = 'none';
      await deletePost(idPost);
      home.appendChild(postRemovedModal());
      post.innerHTML = '';
    });
  });
};
