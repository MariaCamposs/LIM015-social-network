  ...
  element.innerHTML = `
    <section class="userProfile">
      <div class="homePhotoProfile">
        <img class="userPhotoBackground" src="${(user.coverPhotoURL || './assets/img/backgroung-mobile.svg')}"/>
        <img class="userPhoto" src="${(user.photoURL || './assets/img/perrito.jpg')}"/>
      </div>
      <div class="userHomeDescription">
        <h3 class="userHomeName">${user.displayName}</h3>
        <h6 class="userHomeOthers">${(user.namePet || '')} - ${(user.location || '')}</h6>
      </div>
    </section>
  `;
  return element;
