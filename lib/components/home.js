...
import {
  getPosts, getUserToFollow, getUserWithOnSnapshot, updatePost, getUser, addUserInFirestore,
...
const templatePost = (dataPost) => {
  ...
  <section class="postHeader">
    ...
    ${user.uid === dataPost.uid ? '<img class="postIcon more" src="./assets/img/puntos.svg"/>' : ''}
  </section>
...
const addViewPost = (idPost, dataPost) => {
  const post = document.createElement('section');
  post.classList.add('post');
  post.id = idPost;
  post.innerHTML = templatePost(dataPost);
  return post;
};

const likePost = (idPost, dataPost) => {
  const user = JSON.parse(localStorage.getItem('user'));
  ...
  getPosts((querySnapshot) => {
    querySnapshot.docChanges().forEach((change) => {
      if (change.type === 'added') {
        posts.appendChild(addViewPost(change.doc.id, change.doc.data()));
        likePost(change.doc.id, change.doc.data());
      }
      if (change.type === 'modified') {
        const idPost = document.getElementById(change.doc.id);
        idPost.innerHTML = templatePost(change.doc.data());
        likePost(change.doc.id, change.doc.data());
      }
  ...
  getUserWithOnSnapshot(user.uid, (querySnapshot) => {
    ...
    <section class="userProfile">
      <div class="homePhotoProfile">
        <img class="userPhotoBackground" src="${(dataUser.coverPhotoURL || '')}"/>
        <img class="userPhoto" src="${(dataUser.photoURL || '')}"/>
      </div>
...
const userFollow = (user) => {
  ...
  element.innerHTML = `
    <div class="userToFollow">
      <img class="userPhotoToFollow" src="${(user.photoURL || 'https://firebasestorage.googleapis.com/v0/b/lab-petstagram.appspot.com/o/images%2Fundefined.jpg?alt=media&token=2f87e3e0-e7f1-4718-8dd5-d73338488f20')}"/>
