...
import {
  ...
  getUser, addUserInFirestore, updatePost,
...
const putImageInFirebaseStorage = (photoFile, key, section) => {
  ...
  putImageFile(namePhoto, photoFile)
    ...
    .then((imageURL) => {
      ...
      user.updateProfile({ [key]: imageURL });
...
const updateDataUser = (nameI, photoFileI, coverPhotoI, petNameI, descriptionI,
  ...
  profileForm.addEventListener('submit', (e) => {
    ...
    updateDocDatainFirestore(user.uid, docData) // se actualiza los datos doc, en la colección users
      .then(() => section.appendChild(dataUploadedModal()));
  });
};
    ...
    <form id="profileForm" class="profileForm">
      ...
      <section class="formProfileBox">
        <label class="profileLabel"> Cambiar foto de perfil
            <input class="inputProfilePhoto inputs" id="photoFile" accept="image/*" type="file" name="photo"/>
        </label>
      </section>
      <section class="formProfileBox">
        <label class="profileLabel"> Cambiar foto de portada
          <input class="inputProfilePhoto inputs" id="coverPhoto" accept="image/*" type="file" name="photo"/>
        </label>
      </section>
      <section class="formProfileBox">
      ...
      <button class="formButton"id="updateProfile">Guardar datos</button>
      </section>
    </form>`;
...
const postsDocDataInRealTime = () => {
  ...
  getPostsWithOnSnapshot(user.uid, (querySnapshot) => {
    querySnapshot.docChanges().forEach((change) => {
      if (window.location.hash === '#/home/profile') {
        ...
        if (change.type === 'modified') {
          updatePost(change.doc.id, { name: user.displayName, photo: user.photoURL });
  ...
  userDocDataInRealTime((docData) => {
    section.innerHTML = `
      <section class="header">
        ...
        <section id="mainProfile" class="mainProfile viewModal">
      </section>`;
    logOut(section);
