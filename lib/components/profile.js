...
import { updateDocDatainFirestore, getUserWithOnSnapshot, getPostsWithOnSnapshot } from '../firebase/firestore.js';
...
const putImageInFirebaseStorage = (photoFile, key, section) => {
  ...
  putImageFile(photoFile)
    ...
    .then((imageURL) => {
      user.updateProfile({ photoURL: imageURL }); // se actualiza photoURL del usuario en la autenticación
...
const updateDataUser = (nameI, photoFileI, coverPhotoI, petNameI, descriptionI, breedI, locationI, profileForm, section) => {
  const user = firebase.auth().currentUser;
  profileForm.addEventListener('submit', (e) => {
    ...
    user.updateProfile({ displayName: nameI.value }); // se actualiza el nombre de usuario en la autenticación
    const docData = {
      name: nameI.value,
...
export const editProfile = () => {
  ...
  userDocDataInRealTime((docData) => {
    ...
    updateDataUser(nameI, photoFileI, coverPhotoI, petNameI, descriptionI, breedI, locationI, profileForm, section);
  });
      ...
      <section id="post" class="post">
        <section class="postHeader">
          <div class="postAuthor">
            <img class="photoProfile" src="${(user.photoURL || './assets/img/perrito.jpg')}"/>
            <div class="postUserDescription">
              <h3 class="postUserName">${(user.displayName)}</h3>
