...
const putImageInFirebaseStorage = (photoFile) => {
  ...
  putImageFile(photoFile)
    .then(() => console.log('se subio archivo'))// getPhotoURL(imageProfileRef.fullPath))
    /* .then((url) => {
      // ! aqui debe ir el update del profile
      console.log(url);
    }) */
...
const updateDataUser = (userNameInput, photoFileInput, petNameInput, descriptionInput, breedInput, locationInput, updateProfile) => {
  ...
  updateProfile.addEventListener('click', (e) => {
    ...
    if (photoFileInput.files[0]) {
      imageUrl = getPhotoURL(putImageInFirebaseStorage(photoFileInput.files[0]).fullPath)
        .then((url) => url);
    } else {
      imageUrl = '';
    }
  ...
  section.innerHTML = `
      <form id="profileForm">
        <section class="headerForm">
          <img id="headerPhoto" src="${user.photoURL}">
