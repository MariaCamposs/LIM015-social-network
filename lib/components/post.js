import { addPost, updateDocDatainFirestore } from '../firebase/firestore.js';
...
const putImageInFirebaseStorage = (photoFile, key) => {
  ...
  putImageFile(photoFile)
    ...
    .then((imageURL) => updateDocDatainFirestore('posts', user.uid, { [key]: imageURL }))
...
const savePost = (container) => {
  ...
  const petsUser = JSON.parse(window.localStorage.getItem('user'));
  // --------------
  let displayName;
  let uid;
  let photoURL;
  ...
  btnPost.addEventListener('click', async (event) => {
    ...
    const photoPost = container.querySelector('#photoButton');
    console.log(photoPost.files);
    if (post === '') {
      console.log('Publicacion vacia');
    } else {
      // if (photoPost) putImageInFirebaseStorage(photoPost[0], 'photoPost');
...
export const createPost = () => {
  ...
  element.innerHTML = `
    <form id="login" class="postForm">
      ...
      <img class="photoProfile-post" src="${(user.photoURL || './assets/img/perrito.jpg')}"/>
