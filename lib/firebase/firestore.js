...
export const updateDocDatainFirestore = (collection, docRef, docData) => firebase.firestore()
  .collection(collection).doc(docRef).update(docData);
...
export const addPost = (uid, displayName, photoURL, post, ubication, namePet) => firebase.firestore().collection('posts').doc().set({
  uid,
  name: displayName,
  photo: photoURL,
  post,
...
export const getPosts = (callback) => firebase.firestore().collection('posts').orderBy('timestamp').onSnapshot(callback);
export const getUserToFollow = (uid, callback) => firebase.firestore().collection('users').where('uid', '!==', uid).limit(5)
  .onSnapshot(callback);
export const getCollection = (collection) => firebase.firestore().collection(collection);
